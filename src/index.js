module.exports = function toReadable (number) {
    ...
    var outnum='';
       for(var i=0;i<numarr.length;++i)
       {
        ...
        {
            outnum+=digits[parseInt(numarr[i], 10)]+' '+dozens[0]+' '+digits[parseInt(numarr[i+2], 10)];
            break;
        }
        else if(numarr[i]>0&&numarr[i+1]>0&&numarr[i+2]==0)
        {
            outnum+=digits[parseInt(numarr[i], 10)]+' '+dozens[0]+' '+dozens[parseInt(numarr[i+1], 10)];
            break;
        }
        else if(numarr[i]>0&&numarr[i+1]==0&&numarr[i+2]==0)
        {
        ...
        else if(numarr[i]==1&&numarr[i+1]==8)
        {
            outnum+='eighteen';
            break;
        }
